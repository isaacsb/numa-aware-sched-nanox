...
namespace nanos {
  namespace ext {
    ...
    class DFIFOPolicy : public SchedulePolicy
    {
    public:
      static bool       _usePriority;
      static bool       _useSmartPriority;

      Atomic<unsigned int> _currQ;
      std::tr1::mt19937 _rand;
    private:
      /** \brief DistributedFIFO Scheduler data associated to each thread
       *
       */
      ...
      explicit DFIFOPolicy ( const DFIFOPolicy & );
      const DFIFOPolicy & operator= ( const DFIFOPolicy & );

    public:
      // constructor
      DFIFOPolicy() : SchedulePolicy ( "Cilk" ), _currQ(0), _rand(time(NULL))
      {
      ...
      virtual void queue ( BaseThread *thread, WD &wd )
      {
        ...
        if (wd.getDepth() > 1) {
          ...
          unsigned int thid = (++_currQ)%size;
		
          BaseThread &dest = thread->getTeam()->getThread(thid);
		
          if ( dest.getTeam() != NULL )
            data = (ThreadData *) dest.getTeamData()->getScheduleData();
        }
	      
        data->_readyQueue->push_back ( &wd );
      }
     ...
     *  \sa BaseThread
     */
    WD * DFIFOPolicy::atIdle ( BaseThread *thread, int numSteal )
    {
      WorkDescriptor * wd;
    ...
    bool DFIFOPolicy::_usePriority = true;
    bool DFIFOPolicy::_useSmartPriority = false;

    class DFIFOSchedPlugin : public Plugin
    {
    public:
      DFIFOSchedPlugin() : Plugin( "Distributed Breadth-First scheduling Plugin",1 ) {}
      ...
      virtual void config( Config& cfg )
      {
        ...
        cfg.registerConfigOption ( "schedule-priority", NEW Config::FlagOption( DFIFOPolicy::_usePriority ), "Priority queue used as ready task queue");
        cfg.registerArgOption( "schedule-priority", "schedule-priority" );

        cfg.registerConfigOption ( "schedule-smart-priority", NEW Config::FlagOption( DFIFOPolicy::_useSmartPriority ), "Smart priority queue propagates high priorities to predecessors");
      ...
      virtual void init() {
        sys.setDefaultSchedulePolicy(NEW DFIFOPolicy());
      }
    };
...
DECLARE_PLUGIN("sched-dfifo",nanos::ext::DFIFOSchedPlugin);
